...
import { 
    ...
    FaChevronLeft, FaChevronRight, FaUserMd, FaLightbulb, FaClinicMedical 
} from 'react-icons/fa';
...
const ListaProfesionales = ({ 
    ...
    // --- DEBUG: Confirma en consola que ves "lugares_atencion" ---
    useEffect(() => {
        if (profesionales.length > 0) {
            console.log("--- DATA PROFESIONAL (Backend) ---");
            console.log("Nombre:", profesionales[0].nombre);
            console.log("Sedes (lugares_atencion):", profesionales[0].lugares_atencion);
        }
    }, [profesionales]);
    ...
    const profesionalesFiltrados = profesionales.filter(p => {
        ...
        const especialidad = p.especialidades_nombres?.[0] || ''; // Ajusta si tu serializer devuelve otro nombre
        const matchTexto = nombre.toLowerCase().includes(searchTerm.toLowerCase()) ||
                           especialidad.toLowerCase().includes(searchTerm.toLowerCase());

        // 2. Filtro por Sede (CORREGIDO SEGÚN TU MODELO DJANGO)
        let matchSede = true;
        
        if (sedeSeleccionada && sedeSeleccionada !== "") {
            const idSede = parseInt(sedeSeleccionada);
            
            // LA CLAVE CORRECTA ES 'lugares_atencion'
            const listaSedes = p.lugares_atencion || [];
            
            if (Array.isArray(listaSedes) && listaSedes.length > 0) {
                matchSede = listaSedes.some(item => {
                    // Django puede devolver [1, 2] (IDs) o [{id:1}, {id:2}] (Objetos)
                    // Esta línea soporta ambos casos:
                    const itemId = typeof item === 'object' ? item.id : item;
                    return itemId === idSede;
                });
            } else {
                // Si seleccionaste una sede y el médico no tiene ninguna asignada, se oculta.
                matchSede = false; 
            }
        }
    ...
    return (
        <div className={`h-full flex flex-col transition-all duration-300 ease-in-out bg-white border-r border-gray-200 shadow-xl z-20 ${isOpen ? 'w-80' : 'w-16'}`}>
            
            {/* HEADER */}
            <div className={`shrink-0 bg-gradient-to-r from-blue-900 to-blue-800 text-white flex items-center transition-all duration-300 ${isOpen ? 'p-5 justify-between' : 'p-2 justify-center py-4'}`}>
                {isOpen && (
                ...
                <button onClick={() => setIsOpen(!isOpen)} className={`text-white hover:bg-white/20 rounded-lg transition-colors p-2 ${!isOpen ? 'w-10 h-10 flex items-center justify-center' : ''}`}>
                ...
                <>
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-2 scrollbar-thin scrollbar-thumb-gray-200 animate-fadeIn">
                        {profesionalesFiltrados.length === 0 ? (
                            <div className="text-center p-8 text-gray-400 text-sm italic border-2 border-dashed border-gray-100 rounded-xl m-2">
                                <FaUserMd className="mx-auto mb-2 text-gray-300" size={24}/>
                                <p>No se encontraron médicos.</p>
                                {sedeSeleccionada && <p className="text-xs text-orange-400 mt-2">Ninguno en esta sede.</p>}
                            </div>
                        ) : (
                            profesionalesFiltrados.map(prof => {
                                ...
                                return (
                                    <div
                                        ...
                                        className={`p-3 rounded-xl cursor-pointer border transition-all duration-200 flex items-center gap-3 relative group ${
                                            isSelected
                                    ...
                                    >
                                        <div
                                            className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shadow-inner shrink-0 transition-all duration-300 ${
                                                isSelected
                    ...
                    <div className="p-4 border-t bg-gray-50 shrink-0 animate-fadeIn">
                        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-3 text-[10px] text-yellow-800 shadow-sm leading-tight">
                            ...
                            <p>• <b>Clic derecho</b> para menú.</p>
                        </div>

                        <button
                            onClick={onOpenModal}
                            disabled={selectedProfs.length === 0}
                            className={`w-full py-3 px-4 rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 transition-all text-sm ${
                        ...
                        >
                            <FaExternalLinkAlt size={12}/> {selectedProfs.length > 0 ? `Ver Agenda (${selectedProfs.length})` : 'Ver Agenda'}
                        </button>
                    </div>
                </>
            )}
            ...
            {!isOpen && (
                <div className="flex-1 flex flex-col items-center py-4 bg-gray-50 space-y-4 animate-fadeIn">
                    ...
                    {selectedProfs.length === 0 && <FaUserMd className="text-gray-300 text-2xl"/>}
                    {selectedProfs.length > 0 && (
                        <button
                            onClick={onOpenModal}
