...
class DisponibilidadViewSet(viewsets.ModelViewSet):
    ...
    def _obtener_citas_activas(self, profesional_id, fecha_inicio, fecha_fin=None):
        try:
            f_ini_str = fecha_inicio.strftime('%Y-%m-%d') if isinstance(fecha_inicio, (date, datetime)) else fecha_inicio
            params = {
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            ...
            if instance.fecha is None:
                ...
                return Response({"mensaje": f"Serie finalizada. Se limpiaron {count_bloqueos} bloqueos futuros huérfanos."}, status=200)
            ...
            # --- CASO B: Es una fecha ESPECÍFICA (Override) ---
                ...
                ocupado = any(instance.hora_inicio <= datetime.strptime(c['hora_inicio'], '%H:%M:%S').time() < instance.hora_fin for c in citas)
...
class SlotGeneratorView(APIView):
    def get(self, request):
        ...
        if not profesional_id or not fecha_str: return Response({"error": "Faltan parámetros"}, status=400)

        fecha_obj = datetime.strptime(fecha_str, '%Y-%m-%d').date()
        dia_semana = fecha_obj.weekday()

        # 1. BLOQUEOS
        if BloqueoAgenda.objects.filter(profesional_id=profesional_id, fecha_inicio__lte=fecha_obj, fecha_fin__gte=fecha_obj).exists():
        ...
        horarios = Disponibilidad.objects.filter(
            ...
            (models.Q(fecha__isnull=True) & (models.Q(fecha_fin_vigencia__isnull=True) | models.Q(fecha_fin_vigencia__gte=fecha_obj)))
        )
        ...
        for horario in horarios:
            ...
            while cursor + timedelta(minutes=duracion) <= fin_turno:
                ...
                if not ocupado: slots_disponibles.append(s_ini)
