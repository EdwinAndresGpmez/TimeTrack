...
class Command(BaseCommand):
    help = "Creates tenant schema and clones appointments domain tables."
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                WHERE table_schema = 'public'
                  ...
                  AND table_name LIKE 'gestion_citas_%'
                ORDER BY table_name
                """
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Schema '{schema}' preparado con {len(tables)} tablas de appointments."
            )
        )
