        ...
        try:
            with transaction.atomic():
                ...
                try:
                    ...
                    return Response(
                        {"detalle": "Formato de fecha/hora inválido."},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                try:
                    ...
                    return Response(
                        {"detalle": "Formato de hora inválido"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                cruce_paciente = (
                    ...
                    .filter(hora_inicio__lt=hora_fin_time, hora_fin__gt=hora_ini_time)
                    .exists()
                )
                ...
                if citas_dia.count() >= config.max_citas_dia_paciente:
                    return Response(
                        {"detalle": "Límite diario de citas alcanzado."},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                if not config.permitir_mismo_servicio_dia:
                    if citas_dia.filter(servicio_id=servicio_id).exists():
                        return Response(
                            {"detalle": "Ya tiene una cita para este servicio hoy."},
                            status=status.HTTP_400_BAD_REQUEST,
                        )
