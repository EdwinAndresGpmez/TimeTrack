...
class Banner(models.Model):
    ...
    # Imagen para Desktop
    imagen_desktop = models.ImageField(
        upload_to="banners/", verbose_name="Imagen PC (1920x1080)"
    )
    # Imagen para Móvil (Opcional pero recomendado para responsive)
    imagen_movil = models.ImageField(
        upload_to="banners/", blank=True, null=True, verbose_name="Imagen Móvil"
    )

    link_accion = models.URLField(
        blank=True, null=True, help_text="Si el banner lleva a algún lado"
    )
...
class VideoGaleria(models.Model):
    ...
    url_externa = models.URLField(
        blank=True, null=True, help_text="Link de YouTube/Vimeo"
    )
